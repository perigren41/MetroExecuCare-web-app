  ...
  return (
    <>
      ...
      <div className="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-3xl shadow-lg w-full max-w-xl max-h-[90vh] relative overflow-y-auto mx-auto my-auto modal-scrollbar">
          {/* Header */}
          <div
            className="flex justify-between items-center 
              bg-[linear-gradient(to_right,#3F6EC0_2%,#00539F_30%,#5D3EA4_50%,#7940A8_75%)] 
              text-white px-4 py-1 rounded-t-lg sticky top-0 z-10"
          >
          ...
          <form onSubmit={handleSubmit} className="p-5">
            ...
            <div className="hidden md:grid grid-cols-[7fr_14fr_auto] grid-rows-[auto] gap-1 text-xs items-start text-left">
              {/* First Name */}
            ...
            <div className="block md:hidden text-xs">
              ...
              {/* Form Fields - Mobile (List, Left-aligned) */}
              <div className="space-y-3">
                {/* First Name */}
                <div className="text-left">
                  <label className="block font-medium text-blue-900 mb-1">First Name</label>
                  <input
                    type="text"
                    name="FirstName"
                    value={formData.FirstName}
                    onChange={handleChange}
                    required
                    className={`border rounded-sm py-1 text-xs w-full px-2 ${borderClass("FirstName")}`}
                  />
                </div>

                {/* Middle Name */}
                <div className="text-left">
                  <label className="block font-medium text-blue-900 mb-1">Middle Name</label>
                  <input
                    type="text"
                    name="MiddleName"
                    value={formData.MiddleName}
                    onChange={handleChange}
                    required
                    className={`border rounded-sm py-1 text-xs w-full px-2 ${borderClass("MiddleName")}`}
                  />
                </div>

                {/* Last Name */}
                <div className="text-left">
                  <label className="block font-medium text-blue-900 mb-1">Last Name</label>
                  <input
                    type="text"
                    name="LastName"
                    value={formData.LastName}
                    onChange={handleChange}
                    required
                    className={`border rounded-sm py-1 text-xs w-full px-2 ${borderClass("LastName")}`}
                  />
                </div>

                {/* Employee ID */}
                <div className="text-left">
                  <label className="block font-medium text-blue-900 mb-1">Employee ID</label>
                  <input
                    type="text"
                    name="employeeid"
                    value={formData.employeeid}
                    onChange={handleChange}
                    className={`border rounded-sm py-1 text-xs w-full px-2 ${borderClass("employeeid")}`}
                  />
                </div>

                {/* Password */}
                <div className="text-left">
                  <label className="block font-medium text-blue-900 mb-1">Password</label>
                  <div className="relative">
                    <input
                      ...
                      value={formData.password}
                      onChange={handleChange}
                      className={`border rounded-sm py-1 text-xs w-full pr-8 px-2 ${borderClass("password")}`}
                    />
                    <button
                      type="button"
                      onClick={() => setShowPassword(!showPassword)}
                ...
                {/* Role */}
                <div className="text-left">
                  <label className="block font-medium text-blue-900 mb-1">Role</label>
                  <select
                    name="role"
                    value={formData.role}
                    onChange={handleChange}
                    className={`border rounded-sm py-1 text-xs w-full shadow-lg px-2 ${borderClass("role")}`}
                  >
                ...
                {/* Position */}
                <div className="text-left">
                  <label className="block font-medium text-blue-900 mb-1">Position</label>
                  <input
                    type="text"
                    name="position"
                    value={formData.position}
                    onChange={handleChange}
                    required
                    className={`border rounded-sm py-1 text-xs w-full px-2 ${borderClass("position")}`}
                  />
                </div>

                {/* Branch */}
                <div className="text-left">
                  <label className="block font-medium text-blue-900 mb-1">Branch</label>
                  <select
                    name="branch"
                    value={formData.branch}
                    onChange={handleChange}
                    className={`border rounded-sm py-1 text-xs w-full shadow-lg px-2 ${borderClass("branch")}`}
                  >
                ...
                <div className="text-left">
                  ...
                  <input
                    ...
                    value={formData.email}
                    onChange={handleChange}
                    className={`border rounded-sm py-1 text-xs w-full px-2 ${borderClass("email")}`}
                  />
                </div>

                {/* Contact */}
                <div className="text-left">
                  <label className="block font-medium text-blue-900 mb-1">Contact Number</label>
                  <input
                    type="text"
                    name="contact_number"
                    value={formData.contact_number}
                    ...
                    className={`border rounded-sm py-1 text-xs w-full px-2 ${borderClass("contact_number")}`}
                  />
                </div>

                {/* Department */}
                <div className="text-left">
                  <label className="block font-medium text-blue-900 mb-1">Department</label>
                  <input
                    type="text"
                    name="department"
                    value={formData.department}
                    onChange={handleChange}
                    className={`border rounded-sm py-1 text-xs w-full px-2 ${borderClass("department")}`}
                  />
                </div>

                {/* Birth Date */}
                <div className="text-left">
                  <label className="block font-medium text-blue-900 mb-1">Birth Date</label>
                  <input
                    type="date"
                    name="birthDate"
                    value={formData.birthDate}
                    onChange={handleChange}
                    className={`border rounded-sm py-1 text-xs w-full px-2 ${borderClass("birthDate")}`}
                  />
                </div>
              </div>

              {/* Actions - Mobile (Centered) */}

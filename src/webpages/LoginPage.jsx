...
import metrobankicon from "@/assets/MBLogo_White.png";
import mainLogo from "@/assets/mainLogo-foreground.svg";

import { mockUsers } from "../webpages/mockUser";
...
export default function LoginPage() {
  ...
  const handleSubmit = (e) => {
    ...
    // look for a matching user from the mockUsers array
    const foundUser = mockUsers.find(
      (u) => u.username === username && u.password === password
    );

    if (foundUser) {
      // All HR positions go to the same dashboard but pass user data
      if (foundUser.role === "HR") {
        navigate("/hr-dashboard", { state: { user: foundUser } });
      } else {
        // fallback if no specific page
        navigate("/dashboard", { state: { user: foundUser } });
      }
    } else if (username === "admin" && password === "1234") {
      // still allow admin login
      ...
      <div className="relative z-10 w-full max-w-200 bg-transparent p-10 rounded-2xl text-white">
        ...
        <div className="text-left mb-8">
          ...
          <p className="text-sm text-gray-200 mt-2">
            Where health meets convenience. Sign in to get started with your
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button
            type="submit"
            className="w-50 bg-white text-blue-700 font-semibold py-2 rounded-full hover:bg-blue-100 transition-colors cursor-pointer"
          >
            LOGIN
          </button>
        </form>

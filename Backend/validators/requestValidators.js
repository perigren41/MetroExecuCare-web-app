...
const validateRequestQuery = (req, res, next) => {
  const { page, limit, status, request_type, priority, assigned_to_me } = req.query;
  const errors = [];
  ...
  if (status && ![
    ...
    'welfare_review', 'approved', 'rejected', 'letter_generated', 
...
const validateHRProcessing = (req, res, next) => {
  ...
  if (hospital_contact && !validator.isMobilePhone(hospital_contact, 'any')) {
